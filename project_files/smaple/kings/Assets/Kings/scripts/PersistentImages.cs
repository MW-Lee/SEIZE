...
public class PersistentImages : MonoBehaviour {
    ...
    public void SetSpriteIndex(int index)
    {
        if (index >= sprites.Length || index < 0) {
            Debug.LogWarning("Kings warning (PersistentImages): There is no element "+index.ToString()+" in the sprite list at '" + gameObject.name+"'. Possible values range from 0 to "+(sprites.Length-1).ToString()+". Index is limited to allowed values.");
        }
    ...
    void ActualizeImage() {
        if(sprites.Length > 0)
        {
            mImage.sprite = sprites[spriteIndex];
        }
        else
        {
